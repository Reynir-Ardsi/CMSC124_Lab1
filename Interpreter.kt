...
class Interpreter(sharedGlobals: Environment? = null) {
    val globals = sharedGlobals ?: Environment()
    ...
    private fun defineNatives() {
        ...
        globals.define("refresh_orb", object : AncientCallable {
            ...
            override fun call(interpreter: Interpreter, arguments: List<Any?>): Any? {
                val name = stringify(arguments[0])
                if (cooldowns.containsKey(name)) { cooldowns[name] = 0; println("[REFRESHER] $name is now ready!") }
                return null
            }
        })
    ...
    fun interpret(stmt: Stmt, isRepl: Boolean = false) {
        ...
        try { execute(stmt) }
            ...
            synchronized(System.out) { System.err.println("$lineInfo Game Error: ${error.message}") }
        }
    }
    ...
    private fun execute(stmt: Stmt?) {
        ...
        when (stmt) {
            ...
            is Stmt.ChatWheel -> { val msg = stringify(evaluate(stmt.expression)); synchronized(System.out) { println("[ALL CHAT] $msg") } }
            ...
            is Stmt.Expression -> { val value = evaluate(stmt.expression); if (isRepl) synchronized(System.out) { println(stringify(value)) } }
        }
    }
    ...
    fun executeBlock(statements: Stmt?, env: Environment) {
        ...
        try { this.environment = env; execute(statements) } finally { this.environment = prev }
    }
    ...
    private fun evaluate(expr: Expr): Any? {
        return when (expr) {
            ...
            is Expr.Binary -> {
                if (expr.operator.type == TokenType.OR) { val left = evaluate(expr.left); if (isTruthy(left)) return left; return evaluate(expr.right) }
                if (expr.operator.type == TokenType.AND) { val left = evaluate(expr.left); if (!isTruthy(left)) return left; return evaluate(expr.right) }
                evaluateBinary(expr)
            }
            is Expr.Cast -> evaluateCast(expr)
            
            // Handle READING index (str[0])
            is Expr.Get -> {
                val obj = evaluate(expr.obj)
                val index = evaluate(expr.index)
                if (obj is String && index is Double) {
                    val i = index.toInt()
                    if (i < 0 || i >= obj.length) throw RuntimeError(expr.name, "Index out of bounds.")
                    return obj[i].toString()
                }
                throw RuntimeError(expr.name, "Only strings can be indexed with numbers.")
            }
            
            // Handle WRITING index (str[0] = "k")
            is Expr.Set -> {
                val obj = evaluate(expr.obj)
                val index = evaluate(expr.index)
                ...
                if (obj is String && index is Double) {
                    ...
                    val strVal = stringify(value)
                    if (strVal.isEmpty()) throw RuntimeError(expr.name, "Cannot assign empty string to char.")
                    
                    // Strings are immutable, so we create a new one and update the variable
                    val chars = obj.toCharArray()
                    chars[i] = strVal[0]
                    val newStr = String(chars)
                    
                    environment.assign(expr.name, newStr)
                    return newStr
                }
                throw RuntimeError(expr.name, "Invalid assignment.")
            }
        }
    }
    ...
    private fun evaluateBinary(expr: Expr.Binary): Any? {
        ...
        return when (expr.operator.type) {
            ...
            TokenType.SLASH -> { checkNumberOperands(expr.operator, l, r); if ((r as Double) == 0.0) throw RuntimeError(expr.operator, "Division by zero."); (l as Double) / (r as Double) }
            TokenType.STAR -> { checkNumberOperands(expr.operator, l, r); (l as Double) * (r as Double) }
            TokenType.PLUS -> { if (l is Double && r is Double) l + r else "$l$r" }
    ...
    private fun evaluateCast(expr: Expr.Cast): Any? {
        ...
        if (callee !is AncientCallable) throw RuntimeError(expr.paren, "Can only cast spells or items.")
        ...
        if (callee is AncientSpell) {
            ...
            try { 
                val manaVal = environment.get(Token(TokenType.IDENTIFIER, "p1_mana", null, 0)) 
                if (manaVal is Double) currentScriptMana = manaVal
            } catch (e: Exception) { }

            if (currentScriptMana < manaCost) return null
            if (currentTick < cooldowns.getOrDefault(name, 0)) return null
            cooldowns[name] = currentTick + cd.toInt()
        }
        return callee.call(this, args)
    }

    private fun checkNumberOperand(operator: Token, operand: Any?) { if (operand !is Double) throw RuntimeError(operator, "Operand must be a number.") }
    private fun checkNumberOperands(operator: Token, left: Any?, right: Any?) { if (left !is Double || right !is Double) throw RuntimeError(operator, "Operands must be numbers.") }
